...
class Annotator:
    ...
    def __init__(
        ...
        *,
        ...
        self.digital_refs: Dict[str, easel.DigitalSequence] = {
            rec.id: easel.TextSequence(name=rec.id.encode(), sequence=str(
                rec.seq)).digitize(self.prot_alphabet)
        ...
        self.digital_rnas: Dict[str,
                                List[easel.DigitalSequence]] = defaultdict(list)
        for rec in self.reference_rnas:
            digital = easel.TextSequence(name=rec.id.encode(), sequence=str(
                rec.seq)).digitize(self.dna_alphabet)
    ...
    def _find_orfs(self) -> Tuple[List[SeqRecord], List[Tuple[SeqFeature, str]]]:
        ...
        for seq in self.contigs:
            record = SeqRecord(seq.seq, id=seq.id, name=getattr(
                seq, "name", seq.id), description=getattr(seq, "description", ""))
        ...
        if not training_seqs and self.reference_gbff and isinstance(self.reference_gbff, str) and os.path.isfile(self.reference_gbff):
            try:
                training_seqs = [str(rec.seq) for rec in SeqIO.parse(
                    self.reference_gbff, "genbank")]
            except Exception:
                training_seqs = []
        ...
        for hit_list in results:
            if len(hit_list) > 0:
                ...
                if ref_feature:
                    ...
                    ref_prot = ref_feature.qualifiers.get(
                        "translation", [""])[0]
                    if str(pred_seq).rstrip("*").strip() != str(ref_prot).rstrip("*").strip():
                        variants = [
                            SeqRecord(Seq(str(pred_seq).rstrip("*")),
                                      id=f"prodigal_{ref_locus}", description=""),
                            SeqRecord(Seq(str(ref_prot).rstrip("*")),
    ...
    def _hmm_search_rna(self, rna_id: str, digital_query: easel.DigitalSequence, contigs: List[SeqRecord]) -> List[Tuple[str, str, int, int, int]]:
        results = hmmer.nhmmer(digital_query, [easel.TextSequence(name=rec.id.encode(
        ), sequence=str(rec.seq)).digitize(self.dna_alphabet) for rec in contigs])
    ...
    def annotate(self) -> List[SeqRecord]:
        ...
        with ThreadPoolExecutor(max_workers=self.threads) as executor:
            futures = [executor.submit(self._direct_match_protein, ftr, seq) for (
                ftr, seq) in predicted_features]
        ...
        with ThreadPoolExecutor(max_workers=self.threads) as executor:
            futures = [executor.submit(self._hmm_search_protein, ftr, seq) for (
                ftr, seq) in unmatched.values()]
        ...
        for feature_id, (feature, _seq) in unmatched.items():
            if feature.type == "CDS":
                ...
                feature.qualifiers["note"] = [
                    "No match found during annotation"]
                annotated_features.append(feature)

        # 4) HMMER for RNAs
        with ThreadPoolExecutor(max_workers=self.threads) as executor:
            futures = [
                executor.submit(self._hmm_search_rna, rna_id,
                                digitals[0], self.contigs)
        ...
        if self.show_variants and variant_records:
            ...
            query_variant_records = [
                rec for rec in variant_records if rec.id.startswith("prodigal_")]
            SeqIO.write(query_variant_records, self.variants_fasta, "fasta")
            log(
                f" Variant protein sequences found: {len(query_variant_records)}")
